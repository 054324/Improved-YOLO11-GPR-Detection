...
class MixUp(BaseMixTransform):
    ...
    def __init__(self, dataset, pre_transform=None, p=0.0) -> None:
        ...
        Examples:
            ...
            >>> dataset = YOLODataset("path/to/Multimodal data.yaml")
...
def v8_transforms(dataset, imgsz, hyp, stretch=False):
    ...
    if dataset.use_keypoints:
        ...
        if len(flip_idx) == 0 and hyp.fliplr > 0.0:
            hyp.fliplr = 0.0
            LOGGER.warning("WARNING ⚠️ No 'flip_idx' array defined in Multimodal data.yaml, setting augmentation 'fliplr=0.0'")
        elif flip_idx and (len(flip_idx) != kpt_shape[0]):
            raise ValueError(f"Multimodal data.yaml flip_idx={flip_idx} length must be equal to kpt_shape[0]={kpt_shape[0]}")
